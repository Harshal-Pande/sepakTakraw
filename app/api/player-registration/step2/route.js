...
export async function POST(request) {
  try {
    ...
    let refData = null
    if (supabase) {
      const { data: refDataDb, error: refError } = await supabase
        .from('player_reference_numbers')
        .select('*')
        .eq('reference_number', reference_number)
        .single()
      if (refError || !refDataDb) {
        return NextResponse.json(
      ...
      // Graceful fallback: accept references generated by Step 1 fallback (format SPF-YYYY-XXXXX)
      const refPattern = /^SPF-\d{4}-\d{5}$/
      if (!refPattern.test(reference_number)) {
        return NextResponse.json(
          { success: false, error: 'Invalid reference number format' },
          { status: 400 }
        )
      }
      refData = { reference_number, is_used: false, expires_at: new Date(Date.now() + 7*24*3600*1000).toISOString() }
    }
    ...
    if (supabase) {
      ...
      // Fallback: no DB persistence, but return success so UX isn't blocked
      registration = { id: 'local-fallback', reference_number, status: 'pending' }
    }
